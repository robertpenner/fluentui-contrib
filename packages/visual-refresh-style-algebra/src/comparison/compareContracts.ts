...
const visit = (layered: unknown, semantic: unknown, field: string, differences: ContractDifference[]): void => {
  if (Object.is(layered, semantic)) {
    return;
  }
  if (typeof layered !== 'object' || layered === null || typeof semantic !== 'object' || semantic === null) {
  ...
  const keys = new Set([...Object.keys(layeredRecord), ...Object.keys(semanticRecord)]);
  for (const key of keys) {
    visit(layeredRecord[key], semanticRecord[key], field ? `${field}.${key}` : key, differences);
  }
};
...
export const compareContracts = (
  ...
  visit(normalizeContract(layered), normalizeContract(semantic), '', differences);
  return differences;
};