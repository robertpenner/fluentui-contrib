...
describe('specialization and composition preservation laws', () => {
    ...
    fc.assert(
      fc.property(buttonCaseArbitrary, input => {
        ...
        for (const resolver of resolvers) {
          ...
          expect(compact.geometry.blockSize).toBeLessThanOrEqual(standard.geometry.blockSize);
          ...
          expect(compact.validationObligations).toEqual(standard.validationObligations);
  ...
  it('Law 7: product-specialization preservation', () => {
    fc.assert(
      fc.property(buttonCaseArbitrary, productArbitrary, (generatedInput, product) => {
        const input: ButtonCase = { ...generatedInput, appearance: 'primary' };
        ...
        for (const resolver of resolvers) {
          ...
          expect(productResult.supportedDomain.supportedStates).toEqual(baseline.supportedDomain.supportedStates);
          expect(productResult.capabilities.semanticActionRole).toBe('button');
        }
      }),
      propertyParameters,
    );
    ...
    fc.assert(
      fc.property(buttonCaseArbitrary, input => {
        ...
        for (const direction of ['ltr', 'rtl'] as const) {
          const start = resolveSemanticButton({ ...input, direction, compositionContext: 'splitButtonStart' }).shape;
          const end = resolveSemanticButton({ ...input, direction, compositionContext: 'splitButtonEnd' }).shape;
  ...
  it('Law 9: forced-colors transformation idempotence', () => {
    fc.assert(
      fc.property(buttonCaseArbitrary, generatedInput => {
        const input: ButtonCase = { ...generatedInput, colorMode: 'forcedColors' };
        const stagesWithoutForcedColors = standardOverrideOrder.filter(stage => stage !== 'forcedColors');
        const state = resolveLayeredButtonInOrder(input, stagesWithoutForcedColors);
  ...
  it('Law 10: explicit precedence and independent-stage commutativity', () => {
    fc.assert(
      fc.property(buttonCaseArbitrary, generatedInput => {
        ...
        expect(normalizeContract(resolveLayeredButtonInOrder(input, independentOrder).contract)).toEqual(
          normalizeContract(resolveLayeredButton(input)),
        );

        const wrongProtectedOrder = standardOverrideOrder.filter(stage => stage !== 'forcedColors');
        wrongProtectedOrder.splice(wrongProtectedOrder.indexOf('interaction'), 0, 'forcedColors');
        expect(
          compareContracts(resolveLayeredButtonInOrder(input, wrongProtectedOrder).contract, resolveSemanticButton(input)),
        ).not.toEqual([]);
      }),
      propertyParameters,
    );
  ...
  it('Law 15: conditional-padding coherence and directional mirroring', () => {
    fc.assert(
      fc.property(buttonCaseArbitrary, fc.constantFrom('before' as const, 'after' as const), (input, iconPlacement) => {
        ...
        const ltr = resolveSemanticButton({ ...base, direction: 'ltr' }).geometry;
        const rtl = resolveSemanticButton({ ...base, direction: 'rtl' }).geometry;
