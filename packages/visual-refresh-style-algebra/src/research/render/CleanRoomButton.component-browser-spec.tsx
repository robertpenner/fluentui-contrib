...
test.describe('clean-room visual relationships', () => {
  test('renders the 36 px shared and 32 px Teams reference policies distinctly', async ({
    mount,
  }) => {
  ...
  test('keeps compact controls no taller than standard controls', async ({
    mount,
  }) => {
    const component = await mount(
      <div>
    ...
    const standardHeight = await component
      .getByRole('button')
      .nth(0)
      .evaluate((element) => element.getBoundingClientRect().height);
    const compactHeight = await component
      .getByRole('button')
      .nth(1)
      .evaluate((element) => element.getBoundingClientRect().height);
  ...
  test('exposes forced-colors focus and reconstructed accessible naming', async ({
    mount,
  }) => {
  ...
  test('mirrors logical asymmetric padding and avoids text overflow', async ({
    mount,
  }) => {
    ...
    const component = await mount(
      <div>
        <ResearchButton
          input={{ ...textAndIcon, direction: 'ltr' }}
          label="A supported long label"
        />
        <ResearchButton
          input={{ ...textAndIcon, direction: 'rtl' }}
          label="A supported long label"
        />
      </div>
    );
  ...
  test('keeps outer corners and removes joined corners in RTL split composition', async ({
    mount,
  }) => {
